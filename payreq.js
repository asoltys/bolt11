...
function hrpToSat(hrpString, outputString) {
  ...
  let satoshisBN = divisor ? valueBN.mul(SATS_PER_BTC).div(DIVISORS[divisor]) : valueBN.mul(SATS_PER_BTC);

  if (divisor === 'n' && !valueBN.mod(new BN(10, 10)).eq(new BN(0, 10)) || satoshisBN.gt(MAX_SATS) || divisor === 'p') {
